...
const fs = require('fs');
const { handleWidgetMessage, getWidgetAgentMessages, handleWidgetStart, getWidgetConversationHistory } = require('../controllers/messageController');
...
router.get('/api/widget/history', apiLimiter, async (req, res) => {
  try {
    const { businessId, sessionId, conversationId } = req.query;
    ...
    const result = await getWidgetConversationHistory({ businessId, sessionId, conversationId });
    res.json(result);
  } catch (error) {
